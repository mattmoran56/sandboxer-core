...
#     examples/monorepo.sandboxer.yaml.
...
sandboxer_env_secrets="${SANDBOXER_SECRETS:-/sandboxer/secrets.env}"
if [[ -f "$sandboxer_env_secrets" ]]; then
  ...
  # silently missing.
  while IFS= read -r sandboxer_env_line || [[ -n "$sandboxer_env_line" ]]; do
    [[ -z "$sandboxer_env_line" || "$sandboxer_env_line" == '#'* ]] && continue
    [[ "$sandboxer_env_line" != *=* ]] && continue
    sandboxer_env_name="${sandboxer_env_line%%=*}"
    ...
    # $sandboxer_env_name names is set, empty value included. `[[ -v ]]` says the
    ...
    [[ -n "${!sandboxer_env_name+set}" ]] && continue
    sandboxer_env_value="${sandboxer_env_line#*=}"
    sandboxer_env_quote="${sandboxer_env_value:0:1}"
    ...
    if [[ ${#sandboxer_env_value} -ge 2 &&
      ("$sandboxer_env_quote" == '"' || "$sandboxer_env_quote" == "'") &&
      "$sandboxer_env_value" == *"$sandboxer_env_quote" ]]; then
      sandboxer_env_value="${sandboxer_env_value:1:${#sandboxer_env_value}-2}"
    fi
    export "$sandboxer_env_name=$sandboxer_env_value"
  done < "$sandboxer_env_secrets"
fi
unset sandboxer_env_secrets sandboxer_env_line sandboxer_env_name sandboxer_env_value sandboxer_env_quote

export SANDBOXER_PROJECT="${SANDBOXER_PROJECT:-$(plan .project)}"
export SANDBOXER_DOMAIN="${SANDBOXER_DOMAIN:-sbx.localhost}"

SANDBOXER_DB_DRIVER="$(plan .database.driver none)"
export SANDBOXER_DB_DRIVER
SANDBOXER_DB_NAME="$(plan .database.name "$SANDBOXER_PROJECT")"
export SANDBOXER_DB_NAME
export SANDBOXER_DB_DIR="$SANDBOXER_STATE/data/$SANDBOXER_DB_DRIVER"

case "$SANDBOXER_DB_DRIVER" in
  mysql)
    ...
    # disposable.
    export SANDBOXER_DB_HOST=127.0.0.1
    export SANDBOXER_DB_PORT=3306
    export SANDBOXER_DB_USER="${SANDBOXER_DB_USER:-sandboxer}"
    export SANDBOXER_DB_PASSWORD="${SANDBOXER_DB_PASSWORD:-sandboxer}"
    ;;
  sqlite)
    export SANDBOXER_DB_FILE="$SANDBOXER_DB_DIR/$SANDBOXER_DB_NAME.sqlite"
    ;;
  d1)
    # A directory rather than a file: miniflare owns the layout inside it, and
    # exactly one service may open it (contracts §6.1).
    export SANDBOXER_D1_DIR="$SANDBOXER_DB_DIR"
    SANDBOXER_D1_OWNER="$(plan .database.owner)"
    export SANDBOXER_D1_OWNER
    ;;
esac

if [[ "$(plan .storage.driver none)" == "minio" ]]; then
  export SANDBOXER_S3_ENDPOINT=http://127.0.0.1:9000
  export SANDBOXER_S3_KEY="${SANDBOXER_S3_KEY:-sandboxer}"
  export SANDBOXER_S3_SECRET="${SANDBOXER_S3_SECRET:-sandboxerlocal}"
  export SANDBOXER_S3_REGION="${SANDBOXER_S3_REGION:-us-east-1}"
fi

# One SANDBOXER_URL_<LABEL> per app, because only the container knows both the
...
if [[ -n "${SANDBOXER_SLUG:-}" ]]; then
  ...
  # 443 on that machine.
  sandboxer_env_port="${SANDBOXER_PUBLIC_PORT:+:${SANDBOXER_PUBLIC_PORT}}"
  while read -r sandboxer_env_label; do
    [[ -z "$sandboxer_env_label" ]] && continue
    sandboxer_env_var="SANDBOXER_URL_$(printf '%s' "$sandboxer_env_label" | tr '[:lower:]-' '[:upper:]_')"
    export "$sandboxer_env_var=${SANDBOXER_SCHEME:-https}://$(fqdn "$sandboxer_env_label")${sandboxer_env_port}"
  done < <(jq -r '.services[]?.label // empty' "$SANDBOXER_PLAN" | sort -u)
  unset sandboxer_env_label sandboxer_env_var sandboxer_env_port
fi

# One SANDBOXER_PORT_<ID> per port-holding service, so a project can address a
# sibling without duplicating the port in two files.
while read -r sandboxer_env_record; do
  [[ -z "$sandboxer_env_record" ]] && continue
  sandboxer_env_port=$(field "$sandboxer_env_record" port)
  [[ -z "$sandboxer_env_port" ]] && continue
  sandboxer_env_var="SANDBOXER_PORT_$(printf '%s' "$(svc_id "$sandboxer_env_record")" | tr '[:lower:]-' '[:upper:]_')"
  export "$sandboxer_env_var=$sandboxer_env_port"
done < <(jq -c '.services[]? | select(.kind != "static")' "$SANDBOXER_PLAN")
unset sandboxer_env_record sandboxer_env_port sandboxer_env_var

# --- project environment names ------------------------------------------------
#
# The names above are sandboxer's. A project reads its own — DB_HOST, S3_ENDPOINT,
...
while read -r sandboxer_env_name; do
  [[ -z "$sandboxer_env_name" ]] && continue
  sandboxer_env_value=$(jq -r --arg n "$sandboxer_env_name" '.env[$n] // ""' "$SANDBOXER_PLAN")
  export "$sandboxer_env_name=$(printf '%s' "$sandboxer_env_value" | envsubst)"
done < <(jq -r '.env // {} | keys[]' "$SANDBOXER_PLAN")
unset sandboxer_env_name sandboxer_env_value

export SANDBOXER_ENV_READY=1
