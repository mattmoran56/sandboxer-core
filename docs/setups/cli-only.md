...
description: The ordinary setup — the sandboxer command, Docker, and nothing to log in to — and the one job you have to give a timer.
---

This is the whole of sandboxer: the `sandboxer` command, Docker, and nothing to log in to. The
...
```prompt
Set this machine up to run sandboxer, and start a sandbox from the repository in this directory.

Read docs/getting-started/install.md, then docs/setups/cli-only.md, then work through them.
Once a sandbox is up, add a cron entry that runs `sandboxer expire` every fifteen minutes and tell
...
has no sandboxer.yaml — I will need to write one before anything can start.
```

## What you install

Docker, Node 22, git, and the `sandboxer` command itself.
[Install it](../getting-started/install.md) has the steps.

## `init` prepares the machine and starts the router

`sandboxer init` makes the directories, builds the base image, issues a certificate if mkcert is
...
and sandboxer does not have one.
...
| Remove it, and its database and uploads | `sandboxer down` |
| Stop the container but keep everything | `sandboxer stop <slug>` |
| Start a stopped one again | `sandboxer start <slug>` |
| See every sandbox on the machine | `sandboxer ls` |
| See one in detail, with its URLs | `sandboxer status <slug>` |
| Follow its log | `sandboxer logs <slug> -f` |
| Get a shell inside it | `sandboxer shell <slug>` |
| Rebuild a backend or a front-end | `sandboxer reload <slug> --go` / `--web=app` |
| Re-run its migrations | `sandboxer reload <slug> --migrate` |
| Work on its database | `sandboxer db shell`, `db migrate`, `db seed`, `db snapshot` |
| Exempt one from the idle clock | `sandboxer keep <slug>` / `sandboxer unkeep <slug>` |
| Stop everything past its idle limit, now | `sandboxer expire` |
| Reap sandboxes whose worktree is gone | `sandboxer gc` |
| Reclaim disk from old images and volumes | `sandboxer prune` |
| Edit a project's credentials | `sandboxer secrets list`, `set`, `unset`, `edit` |
| Load them from the project's own `.env` files | `sandboxer secrets import`, then `secrets check` |
| See which config resolved, and to what | `sandboxer config` |
| Check the machine | `sandboxer doctor` |

Managing repositories works the same way: `sandboxer project` and `sandboxer worktree` clone, list,
...
**`sandboxer expire` is the whole of that mechanism, and nothing runs it for you.** sandboxer starts
no daemon and has no reaper of its own, so a machine that never runs `expire` keeps every sandbox
it ever started.

```bash
sandboxer expire --dry-run    # what would be stopped, and why
sandboxer expire              # stop them
```
...
survive, and `sandboxer start <slug>` brings it back in seconds. Nothing is deleted, so running it
...
*/15 * * * * /usr/local/bin/sandboxer expire >> $HOME/sandboxer-expire.log 2>&1
```
...
**A sandbox somebody asked to keep is exempt.** `sandboxer keep <slug>` writes
`~/.sandboxer/state/keep/<project>/<slug>`; `unkeep` removes it. `expire` reads that file, so the
...
often does not include `/usr/local/bin`, so give `sandboxer` its absolute path. If you set
`SANDBOXER_HOME` or `SANDBOXER_WORKSPACE` in your profile, set them in the crontab too — an `expire`
pointed at the default `~/.sandboxer` will not find sandboxes you keep somewhere else. `launchd`
...
## What sandboxer does not do

None of these is a limitation of this setup. sandboxer is a command-line tool, and each of them is a
job for something built on top of it.

| Not here | Why | What you use instead |
|---|---|---|
| A browser terminal | There is no web process to hold a socket | `sandboxer shell <slug>` |
| Buttons that build and migrate | The verbs are the interface | `sandboxer reload`, `sandboxer db migrate` |
...
| A lifetime that enforces itself | Covered above | `sandboxer expire`, on a timer |
...
|---|---|---|
| `SANDBOXER_HOME` | leave it | Default `~/.sandboxer`, deliberately outside any repository |
| `SANDBOXER_WORKSPACE` | a path on another disk, or leave it | The one part of the tree worth moving; default `~/.sandboxer/workspace` |
| `SANDBOXER_DOMAIN` | leave it | Default `sbx.localhost` resolves to loopback with no DNS setup |
| `SANDBOXER_TTL_HOURS` | a number, or leave it | The lifetime a sandbox gets when neither `--ttl` nor `config.yaml` names one |

**Nothing reads a file of these.** There is no `.env` anywhere in sandboxer's own tree and nothing
...
have no hostname` and carries on. The container is fine; the router is not up. Run `sandboxer init`.
...
`sandboxer expire`, then put it on a timer as above.
