...
files. That much has been run against a real daemon. **The rest of §12 has not been built**: no
runtime is created inside a session (§12.4), nothing writes a clone into a work volume (§12.5),
no idle clock reads a workstation's activity (§12.7), and there is no CLI command and no
dashboard route for any of it (§12.6's `/sessions/:session`). Until those land, the worktree
model is still the one a person uses.
