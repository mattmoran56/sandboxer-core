...
clock reads a workstation's activity (§12.7), there is no CLI command, no session scope in §8's
action table, and no socket reaches a workstation. **The browser app does not draw any of it**, so
there is still no way for a person to reach a session without a `curl`. Until that lands, the
worktree model is the one anybody actually uses.
