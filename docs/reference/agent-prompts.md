...
Installs sandboxer and starts a sandbox for the worktree you are in.

**Needs first:** Docker running, and a `sandboxer.yaml` at the worktree root.  
...
Explains sandboxer and judges whether it fits a repository. Changes nothing.

**Needs first:** Nothing. Run it before you install.  
**From:** [What sandboxer is](../introduction.md)
...
Installs the `sandboxer` command and runs `sandboxer init`.
...
**Needs first:** `sandboxer init` has been run.  
...
**Needs first:** A checkout of the sandboxer repository, and `sandboxer init`.  
...
**Needs first:** One repository, and `sandboxer init`.  
...
Writes a `sandboxer.yaml` for a project from nothing.
...
### sandboxer.yaml, field by field

Answers a question about one config field, without guessing.

**Needs first:** Nothing.  
**From:** [sandboxer.yaml, field by field](../configuration/sandboxer-yaml.md)
...
Writes the `secrets` block and gets the project's credentials into sandboxer.
