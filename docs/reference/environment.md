...
Every environment variable sandboxer reads or writes, in three groups. Mixing the groups up is the
...
Group 1 is yours. Group 2 is written for you on every `sandboxer up`. Group 3 never leaves the
...
|---|---|---|
| `SANDBOXER_HOME` | `~/.sandboxer` | Everything sandboxer keeps on the host. Never put it inside a repository |
| `SANDBOXER_WORKSPACE` | `$SANDBOXER_HOME/workspace` | Where managed projects live. Its own variable so the repositories can sit on a different disk |
| `SANDBOXER_DOMAIN` | `sbx.localhost` | The hostname suffix |
| `SANDBOXER_HTTP_PORT` | `80` | Where the router publishes HTTP |
| `SANDBOXER_HTTPS_PORT` | `443` | Where the router publishes HTTPS |
| `SANDBOXER_TTL_HOURS` | `12` | How long a sandbox may sit unused. **`config.yaml` beats this** |
| `SANDBOXER_CACHE_TTL_HOURS` | `24` | How long a cached seed is reused before it is taken again |
| `SANDBOXER_INSTALL` | found by walking up | Where sandboxer itself is checked out |
| `SANDBOXER_IMAGE` | the built project layer | **Skips the project image build entirely** |
| `SANDBOXER_ROUTER_IMAGE` | a pinned Traefik | Override the router image |
...
> [!WARNING] `SANDBOXER_IMAGE` skips the project layer
...
How long a sandbox may sit unused lives in `~/.sandboxer/config.yaml`, which `sandboxer init` writes
...
The key is the project's workspace directory — the name sandboxer calls that project everywhere
else — or the `project:` its own `sandboxer.yaml` declares. Either works.
...
|---|---|---|
| `SANDBOXER_SOURCE_DB_USER` | `root` | Reading the database you are copying **from** |
| `SANDBOXER_SOURCE_DB_PASSWORD` | empty | The same. An empty value omits the flag entirely |
| `SANDBOXER_DB_USER` | `sandboxer` | The user the app connects as, inside the sandbox |
| `SANDBOXER_DB_PASSWORD` | `sandboxer` | Its password |
| `SANDBOXER_DB_ROOT_PASSWORD` | `sandboxer` | Root inside the sandbox's own server |
| `SANDBOXER_DB_NAME` | the project name, non-alphanumerics folded to `_` | The database inside the sandbox |
| `SANDBOXER_MYSQL_IMAGE` | `mysql:<database.version>`, else `mysql:8.4` | The image a dump is restored into on the host |
...
> [!IMPORTANT] `SANDBOXER_DB_ROOT_PASSWORD` is half of a known defect
> The container initialises root without a password; the host driver defaults to `sandboxer`. Every
...
That is `gh`'s own variable. sandboxer passes it to `gh` and `git` by simply being in their
environment.
...
Written to `~/.sandboxer/build/<project>/<slug>.env` on every `up`, and handed to `docker run`. You
do not set these.

| Variable | Present when | Value |
|---|---|---|
| `SANDBOXER_SLUG` | always | **The one variable the container requires** |
| `SANDBOXER_PROJECT` | always | the `project:` name |
| `SANDBOXER_DOMAIN` | always | the domain in use |
| `SANDBOXER_ACCESS` | always | `public` or `private` |
| `SANDBOXER_SCHEME` | always | `http` or `https`, as the router is actually serving |
| `SANDBOXER_PUBLIC_PORT` | always | the router's port when it is not the scheme's default; empty otherwise |
| `SANDBOXER_DB_USER`, `SANDBOXER_DB_PASSWORD` | `driver: mysql` | `sandboxer` / `sandboxer` unless you set them |
| `SANDBOXER_S3_KEY`, `SANDBOXER_S3_SECRET` | `storage.driver: minio` | `sandboxer` / `sandboxer` |
| `SANDBOXER_WITH` | `up --with` was used | the comma-separated list |
| `SANDBOXER_SEED` | a seed source was chosen | `local`, `file` or `fixtures` |
...
**`SANDBOXER_SCHEME` and `SANDBOXER_PUBLIC_PORT` come from the host because only the host knows.** The
router terminates TLS only when a trusted certificate exists on the machine, and it may be published
on non-default ports. The container builds every `SANDBOXER_URL_<LABEL>` from these two, and a URL
...
`~/.sandboxer/secrets/<project>.env` is **bind-mounted read-only** at `/sandboxer/secrets.env`, and
...
- **An edited credential reaches a running sandbox on a restart.** `sandboxer stop` then `start`.
...
> than yours. `sandboxer secrets list` names which variables those are, because nothing else would.
...
| `SANDBOXER_SERVICE` | inside one supervised service, naming itself |
| `SANDBOXER_ENV_READY` | `1`, once the derivation has run. It is what makes sourcing it twice a no-op |
| `SANDBOXER_SANDBOX` | `true`, always. Baked into the base image, so it is how a script tells it is running inside a sandbox at all |

Two more are set on the **migration command** rather than on the whole container:
`SANDBOXER_MIGRATE_SINCE` when `migrate.since` is set, and `SANDBOXER_MIGRATION_LOCK` for `mysql` — an
advisory-lock name nobody else can be holding.

`SANDBOXER_URL_<LABEL>` and `SANDBOXER_PORT_<SERVICE>` upper-case the name and turn hyphens into
underscores. A label `admin-api` becomes `SANDBOXER_URL_ADMIN_API`.

### Two you have to consume yourself

**`SANDBOXER_MIGRATE_SINCE`** — sandboxer cannot guess a runner's flag spelling, so your command has to
reference it:
...
**`SANDBOXER_D1_DIR` and `SANDBOXER_DB_FILE`** — a file-backed runtime writes into the worktree unless
you point it at the sandbox's own directory. Both the migrate command and the owner's serve command
need it. `sandboxer doctor` warns when it cannot see the variable in the command.

<details class="facts">
<summary><b>Fact sheet</b> — three names that are exported and read by nothing</summary>

`SANDBOXER_ACCESS` in group 2, and `SANDBOXER_D1_OWNER` and `SANDBOXER_S3_REGION` in this group, are
...
So the sandbox derives its own addresses under a `SANDBOXER_` prefix, and your project says what it
...
> command you run by hand with `docker exec` does not. `sandboxer shell`, `db shell` and `db snapshot`
> pass them for you, and each container script sources the derivation for itself.

## Setting them

**Nothing reads a file.** sandboxer ships no `.env`, no `docker-compose.yml` and no settings file
that these are read from — `sandboxer` reads them out of its own environment, so you export them in
the shell you run it from.

```bash
export SANDBOXER_DOMAIN=sbx.example.com
sandboxer init
```

`SANDBOXER_DOMAIN` needs nothing locally: `sbx.localhost` already resolves.
...
started at boot has no login shell, and `SANDBOXER_HOME` falling back to a service account's home
