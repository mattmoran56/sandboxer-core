...
export const ROUTER_CONTAINER = "sandboxer-router";
...
export const AUTH_MIDDLEWARE = "sandboxer-auth@file";
 ...
 * sandboxer cannot assume it owns: another local development router, or anything
...
export function routerPorts(env: NodeJS.ProcessEnv = process.env): RouterPorts {
  ...
  return {
    http: read(env.SANDBOXER_HTTP_PORT, 80),
    https: read(env.SANDBOXER_HTTPS_PORT, 443),
  };
}
 ...
 */
export const HANDSHAKE_PATH = "/.sandboxer/auth";

/** The handshake router's name. One per machine, on the dashboard's container. */
export const HANDSHAKE_ROUTER = "sandboxer-handshake";
...
export async function writeRouterConfig(options: {
  ...
  await writeFile(
    config,
    [
      "# Generated by sandboxer init. Edits are lost on the next run.",
      "entryPoints:",
      "  web:",
      '    address: ":80"',
...
export async function startRouter(options: RouterOptions): Promise<void> {
  ...
  const result = await docker.raw(
    routerArgs({
      ...
      bind,
      ports,
      image: (options.env ?? process.env).SANDBOXER_ROUTER_IMAGE,
    }),
  );
  if (result.code !== 0) {
    ...
    throw new RouterError(
      taken
        ? `Something is already listening on ${bind}:${ports.http}` +
          ...
          ".\n" +
          "  Stop it, or give sandboxer other ports:\n" +
