...
// - orphanVolumes: nothing outside the sandboxer prefix is ever considered
...
// - supersededImages: sandboxer/base and sandboxer/dashboard are never superseded
...
// - supersededImages: an image outside the sandboxer namespace is not ours to remove
...
describe("orphan volumes", () => {
  ...
  it("counts a reaped sandbox's volumes as orphans", () => {
    ...
    expect(plan.volumes).toContain("sandboxer-data-acme-tkt-1");
    expect(plan.volumes).toContain("sandboxer-data-acme-tkt-2");
  });
  ...
  it("never counts a survivor's volumes as orphans", () => {
    ...
    expect(plan.volumes).not.toContain("sandboxer-data-acme-tkt-1");
    expect(plan.volumes).not.toContain("sandboxer-www-acme-tkt-1");
    expect(plan.volumes).toContain("sandboxer-data-acme-tkt-2");
  });

  it("leaves the shared volumes alone", () => {
    const plan = planGc({ sandboxes: [], volumes, worktreeExists: alive });
    expect(plan.volumes).not.toContain("sandboxer-gocache");
  });
   ...
   * here is deliberately under `sandboxer-`, where a name-prefix test alone would
   * not save it.
   */
  it("keeps a volume the caller reserved, even when every sandbox is reaped", () => {
    ...
    expect(plan.keep).toEqual([]);
    expect(plan.volumes).not.toContain("sandboxer-jef-agent");
  });

  // And the weaker guarantee, which holds with no `protectVolumes` at all: the
  // engine proposes only names of the two shapes it mints — `sandboxer-<purpose>-…`
  // and `sandboxer-deps-…` — so a name in somebody else's shape is out of scope
  ...
  it("never removes a volume something has mounted", () => {
    const plan = planGc({
      sandboxes: [],
      volumes: ["sandboxer-data-acme-tkt-9"],
      worktreeExists: alive,
      mountedVolumes: new Set(["sandboxer-data-acme-tkt-9"]),
    });
  ...
  it("leaves a dependency volume alone until a mount list proves it unused", () => {
    const withoutMounts = planGc({ sandboxes: [], volumes: ["sandboxer-deps-abc123"], worktreeExists: alive });
    expect(withoutMounts.volumes).toEqual([]);

    const withMounts = planGc({
      sandboxes: [],
      volumes: ["sandboxer-deps-abc123"],
      worktreeExists: alive,
      mountedVolumes: new Set(),
    });
    expect(withMounts.volumes).toEqual(["sandboxer-deps-abc123"]);
  });
});
...
describe("a work volume is never reclaimed", () => {
  const work = "sandboxer-work-eng-3941";
  ...
  it("survives every sandbox on the machine being reaped", () => {
    const plan = planGc({
      sandboxes: [sandbox({ slug: "tkt-1" }), sandbox({ slug: "tkt-2" })],
      volumes: [work, "sandboxer-data-acme-tkt-1", "sandboxer-data-acme-tkt-2"],
      worktreeExists: gone,
      mountedVolumes: new Set(),
    });
    expect(plan.keep).toEqual([]);
    expect(plan.volumes).toEqual(["sandboxer-data-acme-tkt-1", "sandboxer-data-acme-tkt-2"]);
  });
  ...
  it("survives its own session's runtime being reaped beside it", () => {
    ...
    const plan = planGc({
      ...
      volumes: [work, "sandboxer-data-acme-eng-3941-web"],
      worktreeExists: gone,
      mountedVolumes: new Set(),
    });
    expect(plan.volumes).toEqual(["sandboxer-data-acme-eng-3941-web"]);
  });

  it("keeps every session's volume, not just one", () => {
    const plan = planGc({
      sandboxes: [],
      volumes: [work, "sandboxer-work-doc-only", "sandboxer-work-a"],
      worktreeExists: alive,
      mountedVolumes: new Set(),
    });
    expect(plan.volumes).toEqual([]);
...
const older = image({ repository: "sandboxer/acme", tag: "40ed880f9db8" });
const newer = image({
...
describe("superseded images", () => {
  it("offers the older image of a project, and names what replaced it", () => {
    ...
    expect(offered[0]?.reference).toBe("sandboxer/acme:40ed880f9db8");
   ...
   * repository inside the `sandboxer/` namespace deliberately: Jef's real extra
   ...
   * under `sandboxer/`, and this is what they would rely on.
   */
  it("never supersedes a repository the caller reserved", () => {
    const rows = [
      image({ repository: "sandboxer/embedder", tag: "0.1.0" }),
      image({ repository: "sandboxer/embedder", tag: "latest", created: new Date("2026-08-27T00:00:00.000Z") }),
    ];
    expect(supersededImages(rows)).toHaveLength(1);
    expect(supersededImages(rows, ["sandboxer/embedder"])).toEqual([]);
  });
  ...
  // sandboxer gave it addresses it. `docker image prune` owns that set.
  it("leaves a dangling image to docker's own prune", () => {
    expect(supersededImages([image({ repository: "sandboxer/acme", tag: "<none>" }), newer])).toEqual([]);
